...
def update_db(promotions):
    ...
    for promo in promotions:
        cursor.execute('''
            INSERT INTO promotions (platform, name, deal_price, original_price, url, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (promo['platform'], promo['name'], promo['deal_price'], promo['original_price'], promo['url'], datetime.datetime.now()))
    conn.commit()
    conn.close()
