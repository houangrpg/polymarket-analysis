...
def generate_dashboard():
    ...
    # --- Kindle Auto Page (Force Refresh) ---
    kindle_auto_html = f'''<!doctype html><html lang="zh-TW"><head><meta charset="utf-8"><meta http-equiv="refresh" content="60"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Kindle Dash</title><style>body {{ font-family:serif; background:white; color:black; margin:10px; padding:0; }} .k-card {{ border:2px solid black; padding:15px; margin-bottom:15px; }} .k-title {{ font-size:24px; font-weight:bold; border-bottom:1px solid black; margin-bottom:10px; padding-bottom:5px; }} .k-item {{ font-size:20px; margin-bottom:8px; display:flex; justify-content:space-between; }} .k-meta {{ font-size:14px; color:#333; margin-bottom:20px; text-align:right; }} .k-status {{ font-size:18px; line-height:1.4; }} </style></head><body><div class="k-meta">{updated_at} (Auto-Refresh: 60s)</div><div class="k-card"><div class="k-title">系統狀態</div><div class="k-status"><b>Status:</b> {status_data.get("system_status", "Online")}<br><b>Progress:</b> {status_data.get("in_progress", ["None"])[0] if status_data.get("in_progress") else "N/A"}</div></div><div class="k-card"><div class="k-title">熱門套利/異常</div>{"".join([f'<div class="k-item"><span>{m["title"][:15]}...</span><b>{m["edge"]}</b></div>' for m in hot_markets[:5]])}</div><div class="k-card"><div class="k-title">美股聯動</div>{"".join([f'<div class="k-item"><span>{s["s"]}</span><b>{s["c"]}</b></div>' for s in stocks[:5]])}</div><div style="font-size:10px; text-align:center; margin-top:30px;">JoeClowAI Lab Kindle Mode</div></body></html>'''
